...
<template>
  <div class="grid-container">
    <div
      @click="slideShow.session.paymentPlan = option"
      :class="{ selected: slideShow.session.paymentPlan?.name == option.name }"
    ...
    >
      ...
      <code class="frequency-amount"
        >${{ ((slideShow.totalCost / option.duration) * (1 - option.discount)).toFixed(2) }}</code
      >
