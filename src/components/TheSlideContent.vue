...
<template>
  <div class="container column flex">
    <div
      v-if="slideShow.session.status === 'Complete' && slide.type != 'conclusion'"
      class="overlay"
    ></div>
    <h2 v-if="slide.type !== 'introduction' && slide.type !== 'visualization'">
      {{ slide.title }}
    </h2>
    ...
    <TheCustomization
      :customizations="slideCustomizations"
      :slide="index"
