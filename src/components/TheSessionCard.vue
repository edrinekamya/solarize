...
<template>
  <article @click="slidesShow.startExisting(session)">
    <section class="flex column last-session">
      <div @click.stop class="top row spaced">
        <input
          ...
          v-model="main.sessions[session.id as number].customer"
          class="session-name"
        />
    ...
    <div class="session-info row spaced">
      ...
      <section v-if="session.status == 'Active'">
        <progress
          :value="session.progress"
          :max="slidesShow.numOfSlides"
          class="session-progress-bar"
        ></progress>
        {{ session.progress }}/{{ slidesShow.numOfSlides }}
      </section>
